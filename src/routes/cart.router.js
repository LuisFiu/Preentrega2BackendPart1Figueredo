...
router.post("/", async (req, res) => {
  ...
  if (result === -1) {
    return res
      .status(500)
      .send({ status: "error", error: "Couldn't create cart" });
  }
...
router.get("/:cid", async (req, res) => {
  ...
  if (result === -1) {
    return res
      ...
      .send({
        status: "error",
        error: "Carts list is empty, try creating a cart first",
      });
  }
...
router.put("/:cid/product/:pid", async (req, res) => {
  ...
  if (result === -1) {
    return res
      .status(500)
      .send({ status: "error", error: "Couldn't add product to cart" });
  }
  if (result === -2) {
    return res
      .status(400)
  ...
  if (result === -3) {
    return res
      ...
      .send({
        status: "error",
        error: "You are trying to add a product that doesn't exist",
      });
  }
  ...
  if (result === -3) {
    return res
      ...
      .send({
        status: "error",
        error:
          "You are trying to add a product that doesn't exist or repeated product",
      });
  }
...
router.delete("/:cid/product/:pid", async (req, res) => {
  ...
  const result = await cartService.deleteProductByID(cartId, productId);

  if (result === -1) {
    return res
      .status(500)
      .send({ status: "error", error: "Couldn't delete product from cart" });
  }

  if (result === -2) {
    return res
      .status(400)
      .send({ status: "error", error: "Cart doesn's exist" });
  }
  ...
  if (result === -3) {
    return res
      ...
      .send({
        status: "error",
        error: "You are trying to delete a product that doesn't exist in cart",
      });
  }
...
router.delete("/:cid", async (req, res) => {
  ...
  const result = await cartService.clearCartProductsById(cartId);
